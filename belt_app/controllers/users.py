...
def register_user():
    if user.User.create_user(request.form):return redirect('/dashboard')
...
def login():
    if user.User.login_user(request.form):return redirect('/dashboard')
...
def get_single_user(id):
    ...
    if this_user:return render_template('users/user_profile.html', user=this_user)
    flash('This user does not exist')
    return redirect('/users')

# USER TREE MANAGEMENT
@app.route('/user/account')
def manage_user_trees():
    if not session.get('user_id'):return redirect('/')
    users_trees = tree.Tree.fetch_tree_by_user_id(session['user_id'])
    return render_template('/trees/user_trees.html', trees=users_trees)
